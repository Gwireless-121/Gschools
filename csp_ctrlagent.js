...
["_main","_fflush","_initChromeApplication","_optionsPolicyChanged","_sendConfiguration","_saveConfiguration","_onSystemMessage","_initOptionsPage","__embind_initialize_bindings","onRuntimeInitialized"].forEach((prop) => {
...
var wasmBinaryFile;
  wasmBinaryFile = 'csp_ctrlagent.wasm';
...
var ASM_CONSTS = {
  9265996: ($0) => { chrome.runtime.sendMessage( { type: 'sendConfiguration', configuration: UTF8ToString($0) } ) },  
 9266089: ($0, $1) => { chrome.runtime.sendMessage( { type: 'initOptionsPage', applicationName: UTF8ToString($0), isConfigured: $1 } ) },  
 9266200: () => { chrome.runtime.onMessage.addListener( function(request, sender, sendResponse) { if (request.type) { if (request.type === 'SYS') { var msg = _malloc(request.msg.length); HEAPU8.set(intArrayFromString(request.msg), msg); _onSystemMessage(msg); _free(msg); } else if (request.type === 'optionsPageStarted') { _initOptionsPage(); } else if (request.type === 'saveConfiguration') { var configuration = _malloc(request.configuration.length); HEAPU8.set(intArrayFromString(request.configuration), configuration); _saveConfiguration(configuration); } else if (request.type === 'getConfiguration') { var password = _malloc(request.password.length); HEAPU8.set(intArrayFromString(request.password), password); _sendConfiguration(password); } } } ); },  
 9266939: () => { var ___user; chrome.identity.getProfileUserInfo(function (info) { ___user = info.email; var user = _malloc(256); HEAPU8.set(intArrayFromString(___user), user); if (chrome.storage && chrome.storage.managed) { chrome.storage.managed.get('options', function(optionsPolicy) { var ___options = JSON.stringify(optionsPolicy.options); if (___options) { var options = _malloc(___options.length); HEAPU8.set(intArrayFromString(___options), options); _initChromeApplication(user, window.screen.width, window.screen.height, options); _free(options); } else { console.info("JSON Policy is not set - use configuration page."); _initChromeApplication(user, window.screen.width, window.screen.height, 0); } _free(user); }); chrome.storage.onChanged.addListener(function (changes, namespace) { if (namespace == 'managed') { var ___newOptions = JSON.stringify(changes.options.newValue); if (___newOptions) { var newOptions = _malloc(___newOptions.length);; HEAPU8.set(intArrayFromString(___newOptions), newOptions); _optionsPolicyChanged(newOptions); _free(newOptions); } else { console.error("JSON Policy (update) is malformed!"); } } }); } else { _initChromeApplication(user, window.screen.width, window.screen.height, 0); _free(user); } }) },  
 9268165: ($0, $1) => { window.localStorage.setItem(UTF8ToString($0), UTF8ToString($1)); },  
 9268234: ($0, $1) => { var fileNameKey = UTF8ToString($0); var data = window.localStorage.getItem(fileNameKey); if (data != undefined) { stringToUTF8(data, $1, 100000); } },  
 9268386: ($0) => { var msg = UTF8ToString($0); alert(msg); },  
 9268430: ($0) => { var msg = UTF8ToString($0); console.log(msg); },  
 9268480: () => { chrome.runtime.sendMessage({type: 'finishConfiguration'}); },  
 9268539: () => { chrome.windows.create( { 'url':chrome.runtime.getURL('options.html'), top: 0, left: 0, type: "popup", width: screen.width, height: screen.height, focused: true, }); },  
 9268708: ($0, $1, $2, $3) => { var message = UTF8ToString($0); var fs = $1; var lock = $2; window.ui_window_title = UTF8ToString($3); function onUiWindowCreated(uiWindow) { window.ui_window_id = uiWindow.id; if (fs) { chrome.windows.update(uiWindow.id, {focused: true, drawAttention: true, state: "fullscreen"}) } if (lock) { window.ui_interval = window.setInterval(function() { chrome.windows.update(uiWindow.id, {focused: true, drawAttention: true, state: "fullscreen"}); }, 500) } setTimeout(function() { chrome.runtime.sendMessage({type: 'SYS', msg: message}); chrome.runtime.sendMessage({type: 'initUi', ui_window_id: uiWindow.id, ui_window_title: window.ui_window_title}); }, 1000); setTimeout(function() { chrome.runtime.sendMessage({type: 'SYS', msg: message}); chrome.runtime.sendMessage({type: 'initUi', ui_window_id: uiWindow.id, ui_window_title: window.ui_window_title}); }, 3000); }; chrome.windows.onRemoved.addListener(function(windowId) { if (window.ui_window_id == windowId) { window.ui_window_id = undefined; window.clearInterval(window.ui_interval); } }); if (window.ui_window_id == undefined) { chrome.windows.create( { 'url': chrome.runtime.getURL('ctrlagent_ui.html'), top: screen.height / 2 - screen.height * 0.4 | 0, left: screen.width /2 - screen.width * 0.4 | 0, type: "popup", width: screen.width * 0.8 | 0, height: screen.height * 0.8 | 0, focused: true }, onUiWindowCreated) } },  
 9270087: () => { if (window.ui_window_id != undefined) { window.clearInterval(window.ui_interval); chrome.windows.remove(window.ui_window_id); window.ui_window_id = undefined; } },  
 9270248: ($0) => { chrome.windows.create( { 'url': UTF8ToString($0), top: 0, left: 0, type: "popup", width: screen.width, height: screen.height, focused: true }); },  
 9270396: () => { return window.ui_window_id; },  
 9270424: ($0) => { var message = UTF8ToString($0); chrome.runtime.sendMessage({type: 'SYS', msg: message}, function(response) { }); },  
 9270541: ($0) => { Module.DisplayInfoMsgHandler.setDisplayInfo($0, window.screen.width, window.screen.height); },  
 9270637: () => { chrome.runtime.restart(); },  
 9270663: () => { function onLockWindowCreated(lockWindow) { window.lock_window_id = lockWindow.id; window.lock_interval = window.setInterval(function() { chrome.windows.update(lockWindow.id, {focused: true, drawAttention: true, state: "fullscreen"}); }, 500) }; chrome.windows.onRemoved.addListener(function(windowId) { if (window.lock_window_id == windowId) { window.lock_window_id = undefined; window.clearInterval(window.lock_interval); } }); if (window.lock_window_id == undefined) { chrome.windows.create( { 'url': chrome.runtime.getURL('lock.html'), top: 0, left: 0, type: "popup", width: screen.width, height: screen.height, focused: true }, onLockWindowCreated) } },  
 9271318: () => { if (window.lock_window_id != undefined) { window.clearInterval(window.lock_interval); chrome.windows.remove(window.lock_window_id); window.lock_window_id = undefined; } },  
 9271487: () => { if (window.capture_video != undefined) { return; } window.capture_video_request = true; window.capture_onAccessApproved = function onAccessApproved(desktop_id) { if (!desktop_id) { chrome.desktopCapture.chooseDesktopMedia(["screen"], window.capture_onAccessApproved); chrome.runtime.sendMessage({type: 'maximize'}); } else { chrome.runtime.sendMessage({type: 'minimize'}); } navigator.webkitGetUserMedia({ audio: false, video: { mandatory: { chromeMediaSource: 'desktop', chromeMediaSourceId: desktop_id } } }, gotStream, getUserMediaError); function gotStream(stream) { window.capture_stream = stream; window.capture_video.srcObject = stream; } function getUserMediaError(e) { console.log('getUserMediaError: ' + JSON.stringify(e, null, '---')); } }; window.capture_video = document.createElement("VIDEO"); window.capture_video.autoplay = true; window.capture_canvas = document.createElement('CANVAS'); window.capture_canvas.width = window.screen.width; window.capture_canvas.height = window.screen.height; chrome.desktopCapture.chooseDesktopMedia(["screen"], window.capture_onAccessApproved); chrome.runtime.sendMessage({type: 'maximize'}); },  
 9272634: () => { return window.screen.width; },  
 9272666: () => { return window.screen.height; },  
 9272699: ($0) => { var captureAvailable = true; if (window.capture_stream == undefined) { captureAvailable = false; } if (captureAvailable && window.capture_stream.active && window.capture_video_request) { window.capture_video_request = false; } if (captureAvailable && !window.capture_stream.active && !window.capture_video_request) { window.capture_video_request = true; chrome.desktopCapture.chooseDesktopMedia(["screen"], window.capture_onAccessApproved); chrome.runtime.sendMessage({type: 'maximize'}); captureAvailable = false; } if (window.capture_stream == undefined) { captureAvailable = false; } else { captureAvailable = window.capture_stream.active; } if (captureAvailable) { window.capture_canvas.getContext("2d").drawImage(window.capture_video, 0, 0, window.screen.width, window.screen.height); var image = window.capture_canvas.toDataURL("image/png"); if (image != undefined) { var imageString; imageString = _malloc(image.length); HEAPU8.set(intArrayFromString(image), imageString); if (imageString != undefined) { Module.RfbChromeOsScreenControlDevice.handleImage($0, imageString); _free(imageString); } } } else { chrome.tabs.query({ currentWindow: true, active: true }, function(tabs) { if (tabs) { var tab = tabs[0]; if (tab ) { chrome.tabs.captureVisibleTab(null, {}, function (image) { if(chrome.runtime.lastError == undefined) { if (image != undefined) { var imageString; imageString = _malloc(image.length); HEAPU8.set(intArrayFromString(image), imageString); if (imageString != undefined) { Module.RfbChromeOsScreenControlDevice.handleImage($0, imageString); _free(imageString); } } } }); } } }); } },  
 9274308: ($0) => { chrome.tabs.getSelected(null, function(tab) { if (tab) { if (tab.url.startsWith('http')) { Module.RfbChromeOsScreenControlDevice.setHwResolution($0, tab.width, tab.height); } } } ); },  
 9274494: ($0) => { chrome.tabs.getSelected(null, function(tab) { if (tab == undefined) { return; } if (!tab.url.startsWith('http')) { return; } Module.RfbChromeOsScreenControlDevice.setHwResolution($0, tab.width, tab.height); chrome.tabs.captureVisibleTab(null, {}, function (image) { if(chrome.runtime.lastError == undefined) { if (image != undefined) { var imageString; imageString = _malloc(image.length); HEAPU8.set(intArrayFromString(image), imageString); if (imageString != undefined) { Module.RfbChromeOsScreenControlDevice.handleImage($0, imageString); _free(imageString); } } } } ); } ); },  
 9275076: ($0) => { chrome.tabs.onUpdated.addListener(function (tabId, changeInfo, tab) { var urlString = 0; var titleString = 0; if (tab) { if (tab.url != undefined) { urlString = _malloc(tab.url.length); HEAPU8.set(intArrayFromString(tab.url), urlString); } if (tab.title != undefined) { titleString = _malloc(tab.title.length); HEAPU8.set(intArrayFromString(tab.title), titleString); } Module.ChromeWebBrowserBlocker.handleOnUrl($0, tabId, urlString, titleString); if (urlString != undefined) { _free(urlString); }; if (titleString != undefined) { _free(titleString); }; } } ); },  
 9275641: ($0) => { chrome.tabs.remove($0); },  
 9275669: ($0, $1) => { chrome.tabs.update($0, {url: UTF8ToString($1)}); }
};
...
/** @type {function(...*):?} */
var _initChromeApplication = Module["_initChromeApplication"] = createExportWrapper("initChromeApplication");
/** @type {function(...*):?} */
var _optionsPolicyChanged = Module["_optionsPolicyChanged"] = createExportWrapper("optionsPolicyChanged");
/** @type {function(...*):?} */
var _sendConfiguration = Module["_sendConfiguration"] = createExportWrapper("sendConfiguration");
/** @type {function(...*):?} */
var _saveConfiguration = Module["_saveConfiguration"] = createExportWrapper("saveConfiguration");
/** @type {function(...*):?} */
var _onSystemMessage = Module["_onSystemMessage"] = createExportWrapper("onSystemMessage");
/** @type {function(...*):?} */
var _initOptionsPage = Module["_initOptionsPage"] = createExportWrapper("initOptionsPage");
/** @type {function(...*):?} */
...
var ___start_em_js = Module['___start_em_js'] = 9275722;
var ___stop_em_js = Module['___stop_em_js'] = 9276204;
